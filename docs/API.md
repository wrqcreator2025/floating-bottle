...
2. 前端可以连续新建多个瓶子，每个瓶子独立保存且累计数量不限；同一用户最多有 10 个瓶子同时主动寻找过来人。
...
同一个瓶子允许同时投递给多位符合条件的人，分别接住并回信。每人对应独立 Invitation 和 Connection；一个人接住、放行或结束对话，不影响其他人。每用户 10 个主动寻找名额限制的是同时处于 `searching` 的瓶子数量，不是单瓶接收人数。
...
若当前用户已有 10 个主动寻找中的瓶子，返回 `409 ACTIVE_BOTTLE_LIMIT_REACHED`。草稿仍保留，不自动删除或覆盖。因网络重试而对已经 searching 的同一瓶重复调用时，返回当前成功结果，不重复创建匹配任务。
...
仅将 `paused` 恢复为 `searching`，保留已尝试人数和接收者记录，不重新投递给同一人。若 10 个并行搜索名额均已占用，返回 `409 ACTIVE_BOTTLE_LIMIT_REACHED`。返回 `200` 和更新后的 Bottle；因网络重试而对已 searching 的同一瓶重复调用时返回当前结果，不重复创建任务。
...
成功后 `searchRound` 加一、当前轮 `attemptedCount` 归零，瓶子进入 `searching` 并重新占用一个主动名额。历史邀请和连接保留，不再次投递给相同用户。若 10 个并行搜索名额均已占用，返回 `409 ACTIVE_BOTTLE_LIMIT_REACHED`；状态不允许时返回 `409 INVALID_BOTTLE_STATE`。
